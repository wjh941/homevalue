...
LightGBM 建模(含校准预测区间与单房归因) -> 时间外推验证 -> FastAPI 服务 -> 仪表盘 -> A/B 实验设计 -> CI(含重训质量门禁)。
...
| 诚实口径 | **时间外推**(早期训练/后期测试)MAE 7,971 / MAPE 12.43% / R² 0.859——随机切分的乐观偏差 +38.8% 已量化 |
| baseline 提升 | 线性回归 MAE 14,909 -> LightGBM+TE 5,743(**-61.5%**) |
| 预测区间 | p10/p90 分位模型 + **CQR conformal 校准**,独立评估集覆盖率 **79.7%**(校准前 76.6%) |
| 可解释性 | **单房归因**:相对区内典型房源的逐因子偏离(示例:商圈/小区 +43,927 元/㎡) |
